<template>
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="资料分类" prop="ziliaofenlei">
					<el-input v-model="ruleForm.ziliaofenlei" placeholder="资料分类" clearable  :readonly="ro.ziliaofenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="资料分类" prop="ziliaofenlei">
					<el-input v-model="ruleForm.ziliaofenlei" placeholder="资料分类" readonly></el-input>
				</el-form-item>
			</template>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `ziliaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ziliaofenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
